...
wait_for_postgres() {
  ...
  echo "[postgres-init] postgres not reachable" >&2
  return 1
}
...
echo "[postgres-init] applying base schema"
psql -h "${POSTGRES_HOST}" -p "${POSTGRES_PORT}" -U "${POSTGRES_USER}" -d "${POSTGRES_DB}" -v ON_ERROR_STOP=1 -f "${SCHEMA_DIR}/aggregated_ticks.sql"
psql -h "${POSTGRES_HOST}" -p "${POSTGRES_PORT}" -U "${POSTGRES_USER}" -d "${POSTGRES_DB}" -v ON_ERROR_STOP=1 -f "${SCHEMA_DIR}/orderbook_flushes.sql"

echo "[postgres-init] ensuring partitions"
ensure_partitions
